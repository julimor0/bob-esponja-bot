...
async def on_message(message):
    ...
    if isinstance(message.channel, discord.DMChannel):
        async with message.channel.typing():
            ...
            await message.channel.send(random.choice(["¡Estoy listo! ¡Estoy listo! 🧽", "¡El mejor día del mundo! 🍔", "¡Vamos a cazar medusas! 🪼"]))
        return
    if bot.user.mentioned_in(message):
        if message.content.startswith("/") or message.content.startswith("!") or message.content.startswith(")"):
            await bot.process_commands(message)
            return
        respuestas = [f"¡Hola {message.author.mention}! ¡Estoy listo! 🧽", "¡El mejor día del mundo! Wuju! 🥳", "¡Vamos a cazar medusas! 🪼", "¡Vivo en una piña debajo del mar! 🍍", "¡Quiero una Cangreburger! 🍔", "¡Gary! Miau 🐌"]
        async with message.channel.typing():
